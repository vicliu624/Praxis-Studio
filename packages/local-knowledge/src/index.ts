...
export async function writeProjectIntakeReport(projectRoot: string, candidate: DevelopmentGraphCandidate): Promise<void> {
  ...
  const profile = metadata.profile as
    | {
        ...
        moduleCandidates?: { path: string; kind: string; confidence: string }[];
      }
    | undefined;
  const moduleNodes = candidate.graph.nodes.filter((node) => node.id.startsWith("module:"));
  const generatedNodes = candidate.graph.nodes.map((node) => `- ${node.id} (${node.kind}, ${node.knowledgeKind}, ${Math.round(node.progress * 100)}%)`);
  const generatedEdges = candidate.graph.edges.map((edge) => `- ${edge.source} --${edge.kind}--> ${edge.target} (${edge.knowledgeKind}, ${Math.round(edge.progress * 100)}%)`);
  const report = [
    ...
    "## Project Profile",
    "",
    `Project kinds: ${(profile?.projectKinds ?? []).join(", ") || "unknown"}`,
    `Languages: ${(profile?.languages ?? []).join(", ") || "unknown"}`,
    `Frameworks: ${(profile?.frameworks ?? []).join(", ") || "unknown"}`,
    `Build systems: ${(profile?.buildSystems ?? []).join(", ") || "unknown"}`,
    `Package managers: ${(profile?.packageManagers ?? []).join(", ") || "unknown"}`,
    "",
    "## Detected Modules",
    "",
    ...(profile?.moduleCandidates?.length
      ? profile.moduleCandidates.map((module) => `- ${module.path} (${module.kind}, ${module.confidence})`)
      : moduleNodes.map((node) => `- ${String(node.metadata?.path ?? node.id)} (${String(node.metadata?.moduleKind ?? node.kind)}, ${node.confidence})`)),
    "",
    "## Generated Nodes",
    "",
    ...
    "",
    "FACT = local scan.",
    "CANDIDATE / INFERENCE = generated by rules or agent.",
