...
→ docs-backed Project Memory with Git timeline
```
...
Agent Runtime 是行动系统
docs 是设计界面的可维护真相源
docs + Git 版本时间线 是 Project Memory
.distinction 是缓存、trace、索引、任务交接和过渡期运行态
External Agent 是代码施工队
```

Project Memory 的终局定义：

```text
Project Memory = docs 中所有规范化文档的集合 + Git 记录下来的版本时间线。
足以解释项目的一切，都必须最终落在项目文档网络中。
.distinction 不再是终局记忆系统；它只是在 v0.1 和迁移期承担机器缓存、trace、投影索引、任务交接和 legacy compatibility。
```

Praxis 的设计界面铁律：

```text
任何面向用户展示的设计界面，都必须来自格式化、规范化、完备化的项目文档。
UI 可以展示图、列表、时间线和索引，但它们是文档的投影，不是 authority。
.distinction 可以保存可重建 cache、trace、结构化确认记录和迁移期镜像，但不能成为任何新 Project Memory 的唯一来源。
```
...
```text
Markdown
  干净、可读、适合 Git diff 和 LLM 直接理解。

Semantic HTML
  富展示、可交互、可选择 DOM anchor、可叠加解释/证据/风险/代码映射/时间线图层。
```

Semantic HTML 不是通过界面画布或拖拽工具绘制的。它只能由 agent 通过对话和受控 DOM patch 维护。Design Explorer 负责渲染 HTML、保留 `data-praxis-*` 语义属性、让用户选择 anchor，并把选择交给 agent 解释或修改。
...
→ 规则识别 ProjectProfile
→ Design Discovery 恢复候选业务故事与 Use Case Diagram 列表
→ Agent 分析模块职责、风险、候选关系
→ 生成 DevelopmentGraphCandidate
→ 用户确认
→ 写入 docs project memory
→ 刷新 transitional .distinction cache / trace
→ 进入 Development Graph Workspace / Design Explorer
→ 选中节点/边与 Agent 对话
→ 生成 Plan / Coding Task
```
...
```text
Create New Project
→ Story Canvas 描述用户、系统和外部世界之间的故事
→ Agent 生成 Interaction Model 候选和 Use Case Diagram
→ 用户确认故事基线
→ 选择项目类型与约束
→ Agent 生成需求拆解
→ Agent 生成架构候选
→ Agent 生成 Development Graph
→ Agent 生成 docs project memory
→ Agent 生成 transitional .distinction runtime state
→ 用户确认
→ 创建工程目录
→ 进入 Development Graph Workspace / Design Explorer
```
...
项目记忆和设计界面的 authority 是项目文档网络及其 Git 时间线，而不是 UI 状态、Mermaid 输出或 `.distinction/views`。例如 Use Case Diagram List 应从 `docs/design/use-case-diagrams-maps.md` 和 `docs/design/use-case-diagrams-maps.html` 这类可维护文档解析，再投影到 Design Explorer；后续 Sequence Diagram、Class Collaboration Diagram、Pattern Map 也必须遵守同一规则。
...
```text
Open Existing Project → Design Discovery → Graph Candidate → Confirm → Workspace / Design Explorer → Explain → Plan → Coding Task
```

```text
Create New Project → Story Canvas → Use Case Diagram → Requirements → Architecture → Graph → Docs Project Memory → transitional .distinction runtime state → Workspace / Design Explorer
```
